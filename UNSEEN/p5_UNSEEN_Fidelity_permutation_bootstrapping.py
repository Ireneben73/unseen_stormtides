...
def permutation_test(group_A, group_B, func, n_permutations=10000, seed=None):
    ...
    combined_data = np.concatenate([group_A, group_B], axis=0)
...
obs=obs_stacked['msl'].values
ensemble=ensemble_stacked['msl'].values
reshaped_ensemble = ensemble.reshape(-1, ensemble.shape[-1])
print('obs:', obs)
print('reshaped_ensemble:', reshaped_ensemble)
print('obs shape:', obs.shape)
print('reshaped_ensemble shape:', reshaped_ensemble.shape)

#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
## Run functions
#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------

# Permutation test for 4 parameters
obs_diff_mean, p_value_mean = permutation_test(obs, reshaped_ensemble, func=np.mean, seed=42)
obs_diff_std, p_value_std = permutation_test(obs, reshaped_ensemble, func=np.std, seed=42)
...
#skewness_pvalue = p_value_skew.flatten()
#kurtosis_pvalue = p_value_kurt.flatten()
...
#skewness_pvalue_reshaped = skewness_pvalue.reshape(len(ensemble_ds['latitude']), len(ensemble_ds['longitude']))
#kurtosis_pvalue_reshaped = kurtosis_pvalue.reshape(len(ensemble_ds['latitude']), len(ensemble_ds['longitude']))
...
result_pvalue = xr.Dataset({
    ...
    #'skewness_pvalue': (('latitude', 'longitude'), skewness_pvalue_reshaped),
    #'kurtosis_pvalue': (('latitude', 'longitude'), kurtosis_pvalue_reshaped),
}, coords={
    'latitude': ensemble_ds['latitude'],
    'longitude': ensemble_ds['longitude']
})

output_directory_pvalue=f'../../model_runs/SEAS5_statistics/independence_tests/{model_folder}/fidelity/pvalues_era5_allyear/'
output_file_pvalue = os.path.join(output_directory_pvalue, f'SEAS5_fidelity_pvalue_lat_{lat_min}_{lat_max}_lon_{lon_min}_{lon_max}_permboots.nc')
