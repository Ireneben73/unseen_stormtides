...
date=sys.argv[1]
end_date=sys.argv[2]
model_run_dir=sys.argv[3]
results_dir=sys.argv[4]


# Open the dataset and remove the first timestep
gtsm_netfile = os.path.join(model_run_dir, 'output', 'gtsm_fine_0000_his.nc')
...
def attrib(dataset, frequency):
    dataset.attrs={'title': f'{frequency} timeseries of total water levels forced with ERA5.', 
              'summary': 'This dataset has been produced with the Global Tide and Surge Model (GTSM) version 4.1. GTSM was forced with wind a pressure fields from the reanalysis dataset ERA5', 
              'date_created': str(datetime.utcnow()) + ' UTC', 
              'contact': 'i.benito.lazaro@vu.nl',
              'institution': 'IVM - VU Amsterdam', #include partners later on., 
              'source': 'GTSMv4.1 forced with ERA5',
              'keywords': 'extratropical cyclone; ERA5; water level; storm surge; tides; global tide and surge model;', 
...
outfile_europe=os.path.join(europe_dir, f'gtsm_EU_{date}_{end_date}.nc')
...
outfile_uk=os.path.join(uk_dir, f'gtsm_UK_{date}_{end_date}.nc')
